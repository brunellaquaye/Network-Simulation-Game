...
export async function editDevice(req:Request, res:Response, next:NextFunction) {
     try {
        ...
        const result =await changeDeviceDetails({id:id, name:name, type:type,ipAddress:ipAddress,pingRate:pingRate,latency:latency,trafficLoad:trafficLoad,status:status,scenarioId:scenarioId});
        if (result === null) return res.status(404).json({status: 404, message: 'No devices found' });
